...
export const initBitsDraft = (): BitsDraftController | null => {
  ...
  const setAuthorPlaceholders = () => {
    if (authorNameEl) authorNameEl.placeholder = defaultAuthorName ? `기본: ${defaultAuthorName}` : '기본: 익명';
    if (authorAvatarEl) {
      authorAvatarEl.placeholder = defaultAuthorAvatar
        ? `기본: ${defaultAuthorAvatar}`
        : '상대 이미지 경로 입력 (예: author/avatar.webp, 비워두면 기본 프로필 사용)';
    }
  };
  ...
  const updateIdentityPill = () => {
    ...
    const displayName = authorName || defaultAuthorName || '익명';
    const label = !authorName && !authorAvatar ? `${displayName} (현재)` : displayName;
    if (identityNameEl) identityNameEl.textContent = label;
    const avatarSrc = authorAvatar || defaultAuthorAvatar;
    const fallback = Array.from(displayName)[0] ?? '?';
  ...
  const collectImages = (): ImageDraft[] | null => {
    ...
    for (const row of getImageRows()) {
      ...
      if (!widthValue || !heightValue) {
        setStatus('이미지를 입력했다면 너비와 높이를 모두 입력해주세요.', 'error');
      ...
      if (!Number.isFinite(width) || width <= 0) {
        setStatus('이미지 너비는 양수여야 합니다.', 'error');
        refs.widthEl.focus();
        return null;
      }
      if (!Number.isFinite(height) || height <= 0) {
        setStatus('이미지 높이는 양수여야 합니다.', 'error');
        refs.heightEl.focus();
        return null;
      }
  ...
  const buildMarkdown = () => {
    ...
    if (authorAvatar === undefined) {
      setStatus('프로필 이미지는 상대 경로만 허용됩니다 (예: author/avatar.webp). public/, /, URL, .., ?, # 사용 불가.', 'error');
  ...
  manualCopyBtn?.addEventListener('click', async () => {
    ...
    if (manualNote) {
      manualNote.textContent = copied ? '초안이 복사되었습니다.' : '텍스트가 선택되었습니다. ⌘C / Ctrl+C로 복사하세요.';
    }
  });
  ...
  manualOpenBtn?.addEventListener('click', () => {
    ...
    showManualCopy(markdown, '초안이 생성되었습니다.');
  });
  ...
  generateBtn?.addEventListener('click', async () => {
    ...
    if (copied) setStatus('초안이 복사되었습니다.', 'success');
    else showManualCopy(markdown, '초안이 생성되었습니다.');
  });
  ...
  downloadBtn?.addEventListener('click', () => {
    ...
    setStatus('초안이 다운로드되었습니다.', 'success');
  });
